...
const Error = ({ errorCode, errorMessage, alertMessage, redirectLink, redirectText }) => {
  const msgRef = useRef();
  ...
  return (
    <main className='relative font-medium'>
      <div className='grid grid-cols-[25%_75%] w-[261px] h-[103px] fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'>
        <h1 className='font-poppins font-semibold text-[22px] leading-[33px] border-r-[1px] border-dove-gray flex items-center'>
          {errorCode}
        </h1>

        <p className='font-poppins text-[12px] leading-[18px] text-light-dove-gray flex items-center justify-end dark:text-wild-sand'>
          {errorMessage}
        </p>
