...
interface Props {
  ...
  showButton: (index: number, containerID: string) => void;
}

const RoomSlider: React.FC<Props> = ({ showButton, hideButton, maxSliderItems }) => {
  ...
  return (
    ...
    >
      {rooms.slice(0, maxSliderItems).map(({ text, bgImage, href }, index) => (
        <div
          key={index}
          onMouseEnter={() => {
            setSlideIndex(index);
